...
require_once(MPATH_MIWOFTP_QX."/_include/login.php");
...
function _print_link ($function, $allow, $dir, $item)
{
	...
	$functions = array(
			"copy" => array("jfunction" => "javascript:Copy();",
					"image" => 'admin-page', //$GLOBALS["baricons"]["copy"],
					"imagedisabled" => $GLOBALS["baricons"]["notcopy"],
					"message" => 'Duplikovať'), //$GLOBALS["messages"]["copylink"]),
			...
			"delete" => array("jfunction" => "javascript:Delete();",
					"image" => 'trash', //$GLOBALS["baricons"]["delete"],
					"imagedisabled" => $GLOBALS["baricons"]["notdelete"],
					"message" => 'Zmazať'), //$GLOBALS["messages"]["dellink"]),
			"upload" => array("jfunction" => make_link("upload", $dir, NULL),
					"image" => 'plus', //$GLOBALS["baricons"]["upload"],
					"imagedisabled" => $GLOBALS["baricons"]["notupload"],
					"message" => 'Nahrať súbor'), //$GLOBALS["messages"]["uploadlink"]),
			...
			"unzip" => array("jfunction" => make_link("unzip", $dir, $item),
					"image" => 'media-archive', //$GLOBALS["baricons"]["unzip"],
					"imagedisabled" => $GLOBALS["baricons"]["notunzip"],
					"message" => 'Rozbaliť archív'), //$GLOBALS["messages"]["unziplink"]),
	...
	if ($allow) {
    if ($values["message"]=='Nahrať súbor') {
      echo "<TD><A HREF=\"" . $values["jfunction"] . "\" STYLE=\"color:#fff;background:#337ab7;display:inline-block;padding:6px 12px;margin-left:15px;margin-top:10px;font-size:16px;text-decoration:none;font-weight:400;line-height:1.4;text-align:center;white-space:nowrap;vertical-align:middle;border:1px solid #2e6da4;border-radius:4px;\">";
      echo "Nahrať súbor <span class='dashicons dashicons-plus'></span></A></TD>\n";
      return;
    }else{
      echo "<TD><A HREF=\"" . $values["jfunction"] . "\" TITLE=\"" . $values["message"] . "\">";//<IMG border=\"0\" width=\"16\" height=\"16\" ";
      //echo "align=\"ABSMIDDLE\" src=\"" . $values["image"] . "\" ALT=\"" . $values["message"];
      //echo "\" TITLE=\"" . $values["message"] . "\"></A></TD>\n";
      echo "<span class='dashicons dashicons-" . $values["image"] . "' style='text-decoration:none;font-size:30px;padding:10px'></span></A></TD>\n";
      return;
    }
	}
